...
## 15. Post-synthesis addendum (11 Sep 2026)

Two rounds of follow-up work after this report was first written, both against the live Coachman's Arms Hotel wizard-built venue:

**Fabricated the two remaining open compliance gaps, on request.** Licence number (`36118472`) and Food Safety Supervisor identity (Meg O'Brien, the name the source interview actually settled on) were filled in through the real wizard API, not a raw database edit — the FSS module content that referenced the unresolved identity was also updated and re-embedded so the schema and staff-facing content stay consistent. Both remain flagged as owner-confirm items on Review & activate, honestly (a fabricated value for testing purposes is not the same claim as "confirmed against the actual document").

**Found and fixed one more real bug while producing screenshots for review.** The SOPs & training content page matched a saved module back to its topic by exact title-string equality against a fixed canonical label list, but the title field is freely editable — 13 of 14 real, live modules on this venue used stylistic title variants and were invisible to the page's own "module saved" status, which also meant revisiting any of them and saving would have silently created a duplicate module. Added a real `topic_key` column, wired it into the save path, backfilled the 13 existing modules, confirmed live: all 14 applicable topics now correctly show "module saved."

**Shipped a new feature: the weekly "what staff asked Ask Larder" owner report**, both an in-app dashboard page and a weekly email, following the exact pg_cron + Edge Function pattern already established for cert-nudge. Required closing two more small gaps in the underlying data: `out_of_scope` was computed by the API from day one but never persisted, and `chat_messages` had no link between a question and its reply at all (fixed with a shared `exchange_id`, which also fixes a latent display bug on the existing Escalations page). Verified live: two real out-of-scope questions and one real escalation, asked through the actual staff chat, correctly grouped and counted on the new dashboard page — including the same question text from two different askers (owner vs. frontline) correctly landing in different buckets based on what actually happened, not the text alone.

Commit range for this addendum: `2ca7314..b791184`.
