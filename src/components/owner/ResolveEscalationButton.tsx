...
export function ResolveEscalationButton({ chatMessageId }: { chatMessageId: string }) {
  ...
  async function resolve() {
    setLoading(true);
    setError(null);
    const res = await fetch(`/api/owner/escalations/${chatMessageId}/resolve`, { method: "PATCH" });
    setLoading(false);
    if (!res.ok) {
